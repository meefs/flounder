...
Begin: resolve the clue, stage the target/security-critical source and official answer-free docs, mainnet-match or source-pin each component, write prepare_manifest.json early, record real_target confirmation requirements, record gaps honestly, and stop only after the manifest has nonempty component rows for staged audit material plus either concrete real-target ground_truth or a source-only not_required_reason.`;
}
