...
When the official policy makes deployment, live exposure, or funds at risk a required gate, make a good-faith sizing attempt for every reproduced row and write impact_inventory.json at the workspace root. The framework does not prescribe how to do this: you decide whether the target calls for reading deployment registries, chain state on a local fork, token balances, package release metadata, product docs, explorer pages, or another real-ground-truth source. Keep it bounded to the evidence needed for pass/fail/unknown. The inventory is evidence, not a strategy checklist: record only what you actually established and the blocker when you cannot establish it. Schema: {"generated_at","policy_kind","items":[{"bug","members":["<finding id>"],"status":"funded|unfunded|unknown|not-applicable|blocked","affected_deployments":[{"network","address","kind","is_live":true|false|"unknown","is_funded":true|false|"unknown","funds_at_risk_usd":"number or unknown","block":"block/height/version if applicable","evidence":"source/command/result","method":"how you established it"}],"blockers":["..."],"confidence":"high|medium|low|unknown"}]}. Reference this inventory from the corresponding adjudication gate and payout_estimate.basis. For an explicitly source-only or pre-mainnet policy, cite the official eligibility and reward terms instead of inventing a live deployment or funds-at-risk requirement. Do not mark recommendation:"submit-candidate" unless every gate listed in engagement_profile.required_gates passes and the permitted evidence requirement is satisfied; use needs-human when any required gate is unsettled.
