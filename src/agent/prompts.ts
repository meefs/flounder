...
Write prepare_manifest.json EARLY after the first usable source/provenance set is staged, and no later than the early checkpoint window. If real-target addresses or docs are still being resolved, write a partial manifest with explicit gaps and rewrite it as you resolve more. Stop only when the workspace contains the authorized target code, official answer-free docs/specs, nonempty component rows for what was staged, and a real_target plan that either has concrete ground_truth for real-target confirmation or a clear source-only not_required_reason. Do not keep fetching low-value dependencies after those audit-critical materials are concrete. Finish with prepare_manifest.json at the workspace root (schema in the finalize step). The staged workspace plus that manifest are the audit's source.`;
