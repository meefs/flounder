...
Consolidate the findings into distinct bugs, reproduce each distinct bug against real ground truth, classify the engagement policy, adjudicate scope/live-impact/known-issue/payout gates when the policy is bounty-like, check novelty/corroboration online (leads only), then write confirm_decision.json before emitting done. Do not write report_*.md in this phase; formal reports are generated by the later Report phase. Respond with one JSON tool action or done object.`;
}
